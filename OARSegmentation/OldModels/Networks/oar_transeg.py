...
from OARSegmentation.OldModels.Nets.base_blocks import ModifiedUnetrUpBlock, ModifiedUnetOutBlock
import OARSegmentation.config as config


class TRANSEG(nn.Module):
    """
    ...
    def __init__(
        ...
        self.out = ModifiedUnetOutBlock(spatial_dims=spatial_dims, in_channels=feature_size, out_channels=out_channels)
...
def test():
    ...
    x = torch.ones(1, 1, config.IMAGE_SIZE, config.IMAGE_SIZE, config.IMAGE_SIZE).cuda()
    s = model(x)
    print(s.shape)
