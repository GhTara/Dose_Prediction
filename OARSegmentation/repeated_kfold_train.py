...
class litAutoSeg(pl.LightningModule):
    def __init__(self, pretrain=False, mode_model=0):
        ...
        if pretrain:
            pre_model = torch.load('/content/drive/MyDrive/thesis/pretrained_models/private_seg_7oar/epoch=399-step=32000.ckpt')
...
def main(pretrain, mode_model):
    ...
    openkbp = get_dataset(path=config.MAIN_PATH + config.TRAIN_DIR, state='train',
                                      size=30, cache=True, )
    ...
    fold_indices = []
    for train_indices, val_indices in kfold.split(openkbp):
        fold_indices.append((train_indices, val_indices))
    
    print(fold_indices)
        
    # Save the fold indices to a file
    with open('fold_indices.pkl', 'wb') as f:
        pickle.dump(fold_indices, f)
        
    # Load the fold indices from a file
    ...
    for fold, (train_indices, val_indices) in enumerate(fold_indices):
        ...
        train_data = Subset(openkbp, train_indices)
        val_data = Subset(openkbp, val_indices)
        ...
        #     dirpath="/content/drive/MyDrive/results_thesis/checkpoints/fold-{}".format(fold),
        ...
        mlflow_logger = MLFlowLogger(
            ...
            # run_id = ''
            run_name = 'k-fold-train-{}'.format(fold)
            )
        ...
        # trainer.fit(net, train_loader, val_loader,
        #             ckpt_path='/content/drive/MyDrive/results_thesis/checkpoints/fold-{}/last.ckpt'.format(fold))
