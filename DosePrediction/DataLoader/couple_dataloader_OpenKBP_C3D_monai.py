...
def prepare_data(files, state, a_min, a_max, cache_num, cache, crop_flag):

    seg_keys = ['CT'] + OAR_NAMES
    dose_keys = PTV_NAMES + ['dose', 'dose_mask']
    final_keys = ['PTV'] + dose_keys + ['CT', 'OARs'] + ['real_dose']
    final_keys2 = final_keys + ['Input', 'GT'] if state == 'test' else ['Input', 'GT']

    transforms = Compose(

        [
            LoadImaged(keys=seg_keys+dose_keys, image_only=True, allow_missing_keys=True),
            Empty2FullOAR(),
            Transposed(keys=dose_keys+OAR_NAMES, indices=[2, 1, 0]),
            ORTransform(),
            NormalizePTVTr(),
            ...
            AddChanneld(keys=final_keys, allow_missing_keys=True) 
            if state != 'test' else AddChanneld(keys=final_keys+OAR_NAMES, allow_missing_keys=True),  # for version = 0.7.0 monai
            ...
            ConcatItemsd(
                keys=['CT', 'PTV'], dim=0, name='Input'
            ),

            ConcatItemsd(
                keys=['OARs', 'dose', 'dose_mask'], dim=0, name='GT'
            ),
...
def get_dataset(path, size, state="train", a_min=-1024, a_max=1500, cache_num=24, cache=False, crop_flag=False):
    ...
    data = prepare_data(files=files[0:size], state=state,
                        a_min=a_min, a_max=a_max, cache_num=cache_num, cache=cache, crop_flag=crop_flag)
