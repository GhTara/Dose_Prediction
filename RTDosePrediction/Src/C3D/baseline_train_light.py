...
class CascadeUNet(pl.LightningModule):
    ...
    def validation_step(self, batch_data, batch_idx):
        ...
        #     input_, roi_size, sw_batch_size, lambda x: self.forward(x)[0]
        # )
...
def main(freez=True):
    ...
    checkpoint_callback = ModelCheckpoint(
        dirpath=config.CHECKPOINT_MODEL_DIR_BASE_FINAL_FREEZ,
    ...
    trainer = pl.Trainer(
        ...
        # callbacks=[bar],
        default_root_dir=config.CHECKPOINT_MODEL_DIR_BASE_FINAL_FREEZ,
        # enable_progress_bar=True,
        # log_every_n_steps=net.check_val,
    )

    # train
    trainer.fit(net, datamodule=openkbp, ckpt_path='/content/drive/MyDrive/results_thesis/final_baseline_freez/last.ckpt')
